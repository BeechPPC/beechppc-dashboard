...
async function main() {
  console.log('Generating sample emails...\n')
  ...
  // Get email recipient from environment or use default
  const recipient = process.env.TEST_EMAIL_RECIPIENT || process.env.EMAIL_USER

  if (!recipient) {
    console.error('Error: No recipient email found. Set TEST_EMAIL_RECIPIENT or EMAIL_USER environment variable.')
    process.exit(1)
  }
  ...
  try {
    // Send alert email
    console.log('Sending Alert Email sample...')
    await sendEmail({
      to: recipient,
      subject: '⚠️ [SAMPLE] 3 Google Ads Alerts Triggered',
      html: alertHtml,
    })
    console.log('✓ Alert email sent successfully\n')

    // Send report template email
    console.log('Sending Report Template Email sample...')
    await sendEmail({
      to: recipient,
      subject: '[SAMPLE] Top Performing Search Terms',
      html: reportHtml,
    })
    console.log('✓ Report template email sent successfully\n')

    console.log('All sample emails sent! Check your inbox.')
  } catch (error) {
